...
![](images/figure1.jpg "TTI 1906 5 1/2 digits multimeter from eBay"){: width="800"}
...
![](images/figure2.jpg "TTI 1906 measuring the 5.0V reference voltage"){: width="800"}
...
![](images/figure3.jpg "TTI 1906 PCB and power supply transformer"){: width="800"}
...
![](images/ma_input.png "TTI 1906 200 uA input circuitry"){: width="800"}
...
![](images/input_fail.png "mA input circuitry faulty components"){: width="800"}
...
![](images/d_location.jpg "mA input circuitry faulty components location on PCB. Soldering signals on D5"){: width="600"}
...
![](images/serial_input.png "Serial input circuitry using 1488 and 1489 line drivers"){: width="900"}
...
![](images/serial_pinout.png "Serial connector signals"){: width="600"}
...
![](images/serial_cable.png "Serial cable pinout"){: width="500"}
...
![](images/arcwin.png "ARC Windows software"){: width="800"}
...
I decided to play a little bit more around with the multimeter serial communication. I wrote a small piece of code in Python to get the instrument identification via GPIB. Full Python source code can be downloaded [HERE]({{ site.url }}{% link files/source_code/van_gpib.py %}).
...
![](images/gpib_out.png "GPIB Python code output"){: width="800"}
_GPIB Python code output_

Going a little further on my Python adventures, I wrote a piece of code to read the data from the instrument and plot it. Full Python source code can be downloaded [HERE]({{ site.url }}{% link files/source_code/van_gpib_gph.py %}).

{% gist d5b27ee495f1b684cc832d3c55d465cb %}

Running the code we can see the voltage reading on screen along with a line graph.

![](images/graph.gif "Reading data via serial port graph"){: width="1100"}
...
I'd say that this eBay experience was great! It's was really a bargain! I've bought a very good instrument for a fraction of the price of a similar instrument. However, bear in mind that things could not go so well, and I could end up expend my money in a useless bunch of scrap.

Before venture yourself to buy stuff to repair from eBay, performed some research to find the equipment service manual, schematic, etc. Also, search on forums to understand how reparable the equipment could be.
...
[^4f67]:
    ...
    ![](images/ad584.jpg "AD584K voltage reference kit"){: width="300"}
...
[^afd6]: On this application the MOV is used as transient suppressor.  When the current in inductive load is interrupted by mechanical contacts or when the fuse blows, the voltage across the contacts builds up and causes arcing which is destructive to the instrument. MOVs can be then applied to prevent initiation of the arc.
    ...
    ![](images/mov.png "MOV used as contact protection"){: width="400"}
...
[^c6df]: This kind of protection solution can be built either with a bridge and one extra diode - as does Fluke on some Multimeter - or with 5 diodes as the case of TTI1906 multimeter. For this given protection circuit, 3 diodes are present all the time with either polarity (±), that way protecting the input.
    ...
    ![](images/ma_input_i.png "5-Diodes bridge flowing currents"){: width="400"}
    ...
    ![](images/d_current.png "5-Diodes overvoltage graph"){: height="200"}
...
[^c909]: The MC1488 / MC1489 are monolithic quad line driver and receiver designed to interface data terminal equipment with data communications equipment in conformance with the specifications of EIA Standard No. EIA−232D (RS-232). They basically work as translator between the RS-232 levels (±12V) to DTL (Diode-Transistor-Logic) levels (0 to VCC - in this case 5 Vdc - ). Both ICs are also compatible with TTL logic.
    ...
    ![](images/driver_pin.png "MC1488/MC1489 RS-232 driver/receiver pinout"){: height="300"}
    _MC1488/MC1489 RS-232 driver/receiver pinout_
...
[^0323]: My multimeter is the first TTI1906 version with blue enclosure. The new version that's also obsolete, has a white enclosure.
    ...
    ![](images/1906_white.png "TTI1906 with white enclosure"){: height="300"}
...
[^2360]: The GPIB interface, sometimes called the General Purpose Interface Bus (GPIB), is a general purpose digital interface system that can be used to transfer data between two or more devices. It is particularly well suited for interconnecting computers and test instruments.
    ...
    ![](images/gpib_ex.png "Example of GPIB command format"){: height="200"}
